...
export default function ApprovalScreen() {
  return (
    <>
      ...
      <div className="grid items-start gap-x-8 gap-y-7 xl:grid-cols-[minmax(0,1.45fr)_minmax(0,1fr)]">
        <div>
          <Section
            title="검토조서 결재"
            note={<span className="mono">{APPROVAL.doc}</span>}
          >
            <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_196px]">
              <div>
                <div className="flex flex-wrap gap-x-0 gap-y-2 text-[12.5px]">
                  {[
                ...
                <ul className="mt-4 space-y-1.5 text-[13px] leading-[1.65]">
                  <li>
                    첨부 :{" "}
                    <b className="font-semibold">{APPROVAL.reviewSheet}</b> —
                    법정 서식이 아니라 사내 검토조서에 붙는 신설 1쪽입니다.
                  </li>
                  <li>
                    입력 상수 <b className="mono">{APPROVAL.total}</b>건 중
                    근거가 기록된 값 <b className="mono">{APPROVAL.recorded}</b>
                    건.
                  </li>
                  <li>
                    발주처 제출은{" "}
                    <b className="font-semibold">발주처가 요구할 때만</b>{" "}
                    합니다.
                  </li>
                </ul>
              ...
              {/* 근거 미상 카운터 */}
              <div className="border-l border-[var(--grid)] pl-5 md:border-l-0 md:border-t-0">
          ...
          >
              ...
              <table className="sheet-table min-w-[760px] text-[12.5px]">
                <thead>
                  <tr className="text-[12px] text-[var(--pencil)]">
                    ...
                    <th className="w-[64px] text-right">적용값</th>
                    <th className="w-[52px]">단위</th>
                    <th className="w-[28%]">사유 한 줄</th>
                    <th className="w-[20%]">근거 문서</th>
                    <th>회사 분포 내 위치</th>
                  </tr>
                </thead>
                <tbody>
                  {APPROVAL.rows.map((r) => (
                    <tr key={r.item}>
                      ...
                      <td className="mono text-[var(--pencil)]">{r.unit}</td>
            ...
            <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
              <p className="max-w-[440px] text-[12px] leading-[1.6] text-[var(--pencil)]">
                남은 두 칸은 작성자와 확인자입니다. 작성자 식별자는 이
                첨부본에만 남고, 사내 대장에는 집계만 남습니다.
              </p>
        ...
        <div>
          ...
          <Section title="이 화면이 넘지 않는 선">
            <ul className="space-y-2 text-[13px] leading-[1.65]">
              ...
              <li>
                <b className="font-semibold">적정성을 판정하지 않습니다.</b>{" "}
                과거 산출물의 옳고 그름을 다루지 않습니다.
              </li>
              <li>
