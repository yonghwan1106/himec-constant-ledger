...
| **규칙 ②** 분포 · 층화 · 표본 수 | 용도 · 연면적 구간 · 발주처 지침 계통별 층화. **같은 묶음 표본이 5건 미만이면 분포를 그리지 않고 원문 링크만 목록으로 제시한다**(패널은 뜬다) |
...
- **계수 단위** — 프로젝트 1건 × 항목 1개 = 1행, 세는 대상은 최종 승인본입니다. 같은 값인지는 **단위 환산 후 유효숫자 규칙**으로 판정하고(여유율 15 % = 1.15), 규칙이 바뀌면 가짓수도 바뀌므로 대장 상단에 **규칙 판본 번호**를 띄웁니다.
- **정답셋** — 파일럿 본부 설계 담당 **2인이 주 2일씩 3주(12인일)** 태깅하고 기술사는 확정만 합니다(3인일). 표본은 대표 계산서 40건과 계산 도구 30종(그중 14종은 칸 보도가 전한 HDT 계열, 예: HDT-302 냉수 절감), 약 **250~600행**입니다.
- **성능 상한과 봉인 표본** — 1주차에 같은 60행을 2인이 겹쳐 태깅해 **우연 일치를 제외한 일치도(κ)**를 재며, 그 값이 이 도구의 성능 상한이고 넘겼다고 주장하지 않습니다. 40건 중 **8건은 봉인**해 마지막 주에 한 번만 열고, **게이트 기준선과 AI ②의 오짝 비율은 봉인 표본에서만** 잽니다.
- **누락률** — 40건 중 5건은 사람이 전수 판독해 **수집기가 후보로도 올리지 못한 상수 행 수**를 세고, 그 누락률을 확정률과 함께 적습니다.
...
- **지표는 둘뿐** — ① 같은 항목에 회사가 쓰고 있는 서로 다른 값의 **가짓수(정수)** ② **사유가 붙은 값의 비율**(월별 추이). 1단계의 ②는 40건 태깅에서 사유가 발견된 행의 비율(**과거 소급 계열**)이고, **신규 입력 계열은 2단계 패널 가동일부터**입니다. 확정된 행에서만 사유 부착을 세므로 확정률은 지표가 아니라 분모를 만드는 수입니다. 정확도는 목표가 아니라 **게이트**이며, 상위 후보 채택률을 봉인 표본에서 실측해 기준선을 정하고 미달 항목군은 추정 카드를 띄우지 않습니다.
- **오탐 처리** — 잘못 묶인 항목은 기술사가 사전에서 분리하고 그 이력이 남습니다. 의미 추정에는 확신 수준을 함께 표시합니다.
- **중단 조건 셋(임계값을 착수 전에 못 박습니다)** — 확정률 = 확정 행 수 ÷ 수집기 후보 행 수. ① 확정률 **70% 미만**이면 두 항목 중 하나로 좁힙니다 ② 시트 위치 표준화가 **4주** 안에 안 끝나면 패널을 미룹니다 ③ 사유 칸이 무의미 문자열로 채워지는 비율 **30% 초과**면 입력 상자를 선택지형으로 바꿉니다.
- **조건부 경로 ④(자원)** — 사내 GPU를 확보하지 못하면 야간 배치를 늘려 40건 규모를 CPU 추론으로 성립시키고, 그것도 어려우면 1단계는 규칙 ①만으로 대장을 완성하고 AI 대조는 2단계 첫 달로 옮깁니다.
...
- 「예시 가」~「예시 바」 도구, 파일명, 셀 좌표, 상수, 소유자, 재검증 기한, 분포와 추이 수치는 **전부 가상**입니다. 추이 그래프에는 「예시 — 목표치가 아닙니다」 라벨을 달았습니다.
